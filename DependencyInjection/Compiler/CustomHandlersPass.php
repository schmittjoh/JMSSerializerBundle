...
class CustomHandlersPass implements CompilerPassInterface
{
    public function process(ContainerBuilder $container)
    {
            ...
            foreach ($tags as $attrs) {
                if (!isset($attrs['type'], $attrs['format'])) {
                    throw new \RuntimeException(sprintf(
                        '%s "%s" %s',
                        'Each tag named "jms_serializer.handler" of service',
                        $id,
                        'must have at least two attributes: "type" and "format".'
                    ));
                }
                ...
                if (isset($attrs['direction'])) {
                    $directionConstant = sprintf(
                        '%s_%s',
                        'JMS\Serializer\GraphNavigatorInterface::DIRECTION',
                        strtoupper($attrs['direction'])
                    );

                    if (!defined($directionConstant)) {
                        throw new \RuntimeException(sprintf(
                            'The direction "%s" of tag "jms_serializer.handler" of service "%s" does not exist.',
                            $attrs['direction'],
                            $id
                        ));
                    }
                ...
                foreach ($directions as $direction) {
                    $method = isset($attrs['method'])
                        ? $attrs['method']
                        : HandlerRegistry::getDefaultMethod($direction, $attrs['type'], $attrs['format']);
        ...
        foreach ($container->findTaggedServiceIds('jms_serializer.subscribing_handler') as $id => $tags) {
            ...
            if (!$ref->implementsInterface('JMS\Serializer\Handler\SubscribingHandlerInterface')) {
                throw new \RuntimeException(sprintf(
                    'The service "%s" must implement the SubscribingHandlerInterface.',
                    $id
                ));
            }
            ...
            foreach (call_user_func(array($class, 'getSubscribingMethods')) as $methodData) {
                if (!isset($methodData['format'], $methodData['type'])) {
                    throw new \RuntimeException(sprintf(
                        '%s "%s" must have a "type", and "format" attribute.',
                        'Each method returned from getSubscribingMethods of service',
                        $id
                    ));
                }
                ...
                foreach ($directions as $direction) {
                    ...
                    $method = isset($methodData['method'])
                        ? $methodData['method']
                        : HandlerRegistry::getDefaultMethod($direction, $methodData['type'], $methodData['format']);
    ...
    private function sortAndFlattenHandlersList(array $allHandlers)
    {
        ...
        self::stableUasort($allHandlers, $sorter);
        $handlers = [];
     ...
     * @return bool
     */
    private static function stableUasort(array &$array, $value_compare_func)
    {
        $index = 0;
