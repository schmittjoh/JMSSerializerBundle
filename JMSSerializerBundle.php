...
use JMS\SerializerBundle\DependencyInjection\Compiler\Debug\DataCollectorPass as DebugDataCollectorPass;
...
class JMSSerializerBundle extends Bundle
{
    public function build(ContainerBuilder $builder)
    {
        $builder->addCompilerPass(
            $this->getServiceMapPass(
                'jms_serializer.serialization_visitor',
                'format',
                static function (ContainerBuilder $container, $def): void {
    ...
    private function getServiceMapPass(string $tagName, string $keyAttributeName, callable $callable)
    {
